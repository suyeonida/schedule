...
#define LIST_END -1
#define LIST_FIRST 0     //#define LIST_FIRST 0

//node definition for linked list
typedef struct node{                            //typedef쓰면 main함수에서 구조체를 선언할때 매번 struct를 쓸필요 없다. 
	int index; //index of the node
	void* obj; //object data
	void* next; //pointer to the next

} node_t;                                      //main함수에서 쓸 때 node_t라고 쓰면됌. 

//Inner functions (cannot used at the outside of this file)
node_t* genNode(void)
{
	node_t* ndPtr;  //node_t 형의 ndPtr 포인터  
...
/*
	...
	input parameters(매개변수) : NONE  넘겨 오는 값을 저장할 그릇 매개변수는 없다.  
	return value : pointer to the list (header node) list의 헤더 노드를 반환한다. 
...
{
	node_t* ndPtr = genNode(); //반환값 ndPtr,, make the header node
...
/*
	description : adding a data object to the list   데이터를 리스트에 추가한다. 
